  ...
  return (
    <div className="container mx-auto p-4 sm:p-6 space-y-8 max-w-[1400px] text-foreground select-none">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-b border-border pb-5">
        <div>
          <h1 className="text-2xl font-bold tracking-tight text-foreground font-serif">Organization Funding</h1>
          <p className="text-sm text-foreground/60 mt-1 font-sans">View all funding records and support our cause.</p>
        </div>
        <Link 
          href="/funding/give"
      ...
      <div className="bg-secondary/40 backdrop-blur-md border border-border rounded-2xl shadow-xs overflow-hidden relative">
        ...
        {error ? (
           <div className="flex justify-center p-12 text-danger font-medium bg-danger/10 border-b border-danger/20">{error}</div>
        ) : funds.length === 0 ? (
          <div className="flex flex-col items-center justify-center p-16 text-center border-t border-border/50 border-dashed m-4 rounded-xl">
            <p className="text-foreground/40 font-medium">No funding records found yet.</p>
          </div>
        ) : (
          <div className="overflow-x-auto scrollbar-thin scrollbar-thumb-muted scrollbar-track-transparent">
            <table className="w-full text-sm text-left">
              ...
              <tbody className="divide-y divide-border/50 text-foreground/80 font-medium">
                {funds.map((fund) => (
                  <tr key={fund._id} className="hover:bg-muted/50 transition-colors group">
                    <td className="px-6 py-4 flex items-center gap-3">
                      ...
                      <span className="font-semibold text-foreground group-hover:text-primary transition-colors">{fund.donorName}</span>
                    </td>
                    <td className="px-6 py-4 text-emerald-600 dark:text-emerald-500 font-bold">${fund.amount.toLocaleString()}</td>
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-2 text-foreground/60">
                        <Calendar className="w-3.5 h-3.5" />
                        {new Date(fund.fundingDate).toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric" })}
                      </div>
                    </td>
                  </tr>
                ))}
