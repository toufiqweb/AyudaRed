...
const Footer = () => {
  ...
  return (
    <footer className="bg-background border-t border-border w-full transition-colors duration-300">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 md:py-16">
        ...
        <div className="grid grid-cols-2 md:grid-cols-6 gap-8 md:gap-4 pb-12 border-b border-border">
          ...
          <div className="col-span-2 md:col-span-3 space-y-4">
            <Link
              href="/"
              className="text-2xl font-bold tracking-tight text-foreground hover:opacity-90 transition"
            >
            ...
            <div className="pt-2 max-w-sm">
              ...
              <form onSubmit={(e) => e.preventDefault()} className="flex gap-2">
                <input
                  ...
                  className="w-full px-3 py-2 text-sm bg-muted/60 border border-border rounded-lg placeholder:text-foreground/40 focus:outline-none focus:ring-1 focus:ring-primary focus:border-primary transition"
                  required
                />
                <button
                  type="submit"
                  className="px-3 py-2 bg-primary text-primary-foreground rounded-lg text-sm font-medium hover:opacity-95 transition"
                >
                  Join
                </button>
              </form>
              ...
              <ul className="space-y-2.5">
                {section.links.map((link) => (
                  <li key={link.name}>
                    <Link
                      href={link.href}
                      className="text-sm text-foreground/80 hover:text-primary transition-colors duration-200 inline-flex items-center group"
                    >
                      <span>{link.name}</span>
        ...
        <div className="pt-8 flex flex-col sm:flex-row items-center justify-between gap-4">
          ...
          <div className="text-xs text-foreground/60 text-center sm:text-left order-2 sm:order-1">
            <p>© {currentYear} Bazaro Inc. All rights reserved.</p>
